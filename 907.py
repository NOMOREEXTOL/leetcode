class Solution():
    ...
    def sumSubarrayMins2(self,arr):
        ...
        """
        inf = max(arr)
        dp = [[inf for _ in range(len(arr) + 1)] for __ in range(len(arr) + 1)]
        # dp 数组的含义是arr的i~j段(闭区间)的最小值为dp[i][j]
        # 递推公式为 if min(arr[i],arr[j]) < dp[i+1][j-1]: dp[i][j] = min(arr[i],arr[j])
    ...
    def sumSubArrayMins3(self,arr):
        """
        针对v2的滚动数组优化，由于dp[i][j] 只与 dp[i+1][j-1]有关
        实测超时
        """
        inf = max(arr)
        dp = [inf for _ in range(len(arr) + 1)]
        ...
        for i in range(len(arr) - 1, -1, -1):
            for j in range(len(arr) - 1,i-1,-1):
                if j == 0:
                    ...
                    minSum += dp[j]
                    continue
                if min(arr[i], arr[j]) < dp[j - 1]:
                    dp[j] = min(arr[i], arr[j])
                else:
                    dp[j] = dp[j - 1]
                minSum += dp[j]
    ...
    def sumSubarrayMins4(self,arr):
        """
        题解：单调栈与贡献法解法
        大体思路如下：
        求出每一个元素的辐射范围，所谓辐射范围，就是以当前元素为最小值的最长区间，那么这个元素对总和的贡献值
        可由下式计算： arr[i] * (i - leftFirst[i]) * (rightFirst[i] - i)
        如[3,7,5,2,1,8,4]求5的辐射范围，由单调栈可解出leftbound = 0,rightbound = 4,而i = 2,那么实际辐射区间为
        [7,5,2] 所包含的子区间则为[7,5],[5],[7,5,2],[5,2] 即为2 X 2 = 4 种
        则总和ans 将各项相加即可

        ac:260ms,17MB(10,30%)
        """

        # 求每一个元素向左第一个小于它的数和向右第一个小于它的数的下标i,j，于是该数所对应的辐射边界即为[i+1,j]
        # 采用单调栈的方法求解边界

        st = []
        rightFirst = [-1] * len(arr) # 求右面第一个小于当前元素的元素下标
        leftFirst = [-1] * len(arr) # 求左面第一个小于当前元素的下标

        # 先找每个元素的右边界
        for i in range(len(arr)):
            if not st:
                st.append(i) # 如果栈为空，则直接入栈
                continue
            if arr[st[-1]] <= arr[i]:
                st.append(i)
            else:
        ...
        while st:
            findIndex = st.pop()
            rightFirst[findIndex] = len(arr)

        # 再找每个元素的左边界 (更正:未通过示例[71,55,82,55],找左边界的时候，相同元素不能越过，否则会出现重复计算)
        for j in range(len(arr) - 1,-1,-1):
            ...
            if arr[st[-1]] < arr[j]:
                ...
                while st and arr[st[-1]] >= arr[j]:
                    findIndex = st.pop()
                    leftFirst[findIndex] = j
                st.append(j)

        while st:
            findIndex = st.pop()
            leftFirst[findIndex] = -1

        ans = 0
        for i in range(len(arr)):
            ans += arr[i] * (i - leftFirst[i]) * (rightFirst[i] - i)
    ...
    def sumSubarrayMins5(self,nums):
        """
        针对v1的改进,其实在求解右边界的时候，左边界已经可以求解出来了，比如求[3,7,5,2,1,8,4]
        求7的右边界时，会把7的下标1出栈，则栈内还有3的下标0，同时当前遍历元素5，则可知7的左边界即为当前栈顶的元素，
        7的右边界则为当前遍历元素
        ac:160ms,17MB(75,30%)
        """
        st = []

        rightFirst = [-1] * len(arr)
        leftFirst = [-1] * len(arr)


        # 注： 与v1一样，要考虑重复计算的问题，本次循环为向右寻找第一个小于当前元素的数，由于左边界默认值为-1（即越过相同元素），那么寻找右边界的时候遇到相同元素就不能越过了
        ...
        for i in range(len(arr)):
            if not st or arr[st[-1]] < arr[i]:
                ...
                while st and arr[st[-1]] >= arr[i]:
                    findIndex = st.pop()
                    rightFirst[findIndex] = i
                    leftFirst[findIndex] = st[-1] if st else -1
                st.append(i)
        ...
        for j in range(len(arr)):
            ans += arr[j] * (j - leftFirst[j]) * (rightFirst[j] - j)
